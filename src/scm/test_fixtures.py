...
def make_github_commit(
    ...
    to an account. ``author_type`` is GitHub's account classification, which it
    always sends alongside a login; pass ``"Bot"`` for an automation account.
    """
    result: dict[str, Any] = {
        "sha": sha,
        "commit": {
    ...
    result["author"] = {"login": author_login, "type": author_type} if author_login is not None else None
...
def make_github_pull_request_commit(
    ...
    ``author_type`` is GitHub's account classification, sent alongside every
    login; pass ``"Bot"`` for an automation account.
    """
    result: dict[str, Any] = {
        "sha": sha,
        "commit": {
...
def make_github_graphql_pr_comments_response(
    ...
    return {
        "repository": {
            "pullRequest": {
                "comments": {
                    "nodes": (issue_comments if issue_comments is not None else [make_github_graphql_issue_comment()]),
                    "pageInfo": {
                ...
                "reviewThreads": {
                    "nodes": (review_threads if review_threads is not None else [make_github_graphql_review_thread()]),
                    "pageInfo": {
...
class BaseTestProvider(Provider):
    ...
    def create_git_tree(
        ...
        return ActionResult(
            data=GitTree(
                sha="newtree123",
                tree=[TreeEntry(path="src/main.py", mode="100644", type="blob", sha="new123", size=100)],
                truncated=False,
            ),
            type="github",
            raw={"headers": None, "data": None},
    ...
    def download_archive(
        self, ref: str, archive_format: str = "tarball", request_options: RequestOptions | None = None
    ) -> bytes:
...
class FakeGitHubApiClient:
    ...
    def __init__(self) -> None:
        ...
        self.archive_link_data: str = (
            "https://codeload.github.com/test-org/test-repo/legacy.tar.gz/refs/heads/main?token=fake"
        )
    ...
    def get_commits(self, repo: str, sha: str | None = None, path: str | None = None) -> list[dict[str, Any]]:
    ...
    def get_tree_full(self, repo_full_name: str, tree_sha: str, recursive: bool = True) -> dict[str, Any]:
    ...
    def list_pull_requests(self, repo: str, state: str = "open", head: str | None = None) -> list[dict[str, Any]]:
    ...
    def update_pull_request(self, repo: str, pull_number: str, data: dict[str, Any]) -> dict[str, Any]:
    ...
    def create_review_request(self, repo: str, pull_number: str, data: dict[str, Any]) -> dict[str, Any]:
        self._record_call("create_review_request", repo, pull_number, data)
        self._maybe_raise()
        return {}

    def create_review_comment(self, repo: str, pull_number: str, data: dict[str, Any]) -> dict[str, Any]:
    ...
    def update_check_run(self, repo: str, check_run_id: str, data: dict[str, Any]) -> dict[str, Any]:
...
class SourceCodeManager(Facade):
    ...
    def __new__(cls, provider: Provider, *args, **kwargs) -> "SourceCodeManager":
