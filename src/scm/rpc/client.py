...
def fetch_provider(client: ApiClient, organization_id: int, repository: Repository) -> Provider | None:
    ...
    if repository["provider_name"] == "github":
        ...
        if not installation_id:
            raise RpcInvalidGrant(
                detail="installation_id is required for cursor_origin repositories",
            )

        return CursorOriginProvider(client, organization_id, repository, installation_id=installation_id)
    else:
        return None
